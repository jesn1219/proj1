...
#include "srv0start.h"
#include "trx0purge.h"
#include "calclock.h" // jesnk : header for time profile

// jesnk : declare var : for time profile
unsigned long long buf_dblwr_init_or_load_pages_time, buf_dblwr_init_or_load_pages_count;
...
// jesnk : ! AT buf_dblwr_init
// TESTING_AREA : 12.01.01:13 : check address of block1 and block2. to compare each other
...
//jesnk : testing block1, block2  addresses
printf("block1 : %lu\n",buf_dblwr->block1); // jesnk
  buf_dblwr->block2 =
      mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_BLOCK2);
printf("black2 : %lu\n",buf_dblwr->block2); // jesnk
...
//jesnk : AT buf_dblwr_init_or_load_pages
//jesnk : redef : buf_dblwr_init_or_load_pages : for time profiling
...
dberr_t buf_dblwr_init_or_load_pages(pfs_os_file_t file, const char *path) {
	//jesnk
	printf("jesnk : AT buf_dblwr_init_or_load_pages ... it can call buf_dblwr_init_or_load_pages_internal\n");
	dberr_t ret;

	struct timespec local_time[2];

	clock_gettime(CLOCK_MONOTONIC, &local_time[0]);
	ret = buf_dblwr_init_or_load_pages_internal(file,path);
	clock_gettime(CLOCK_MONOTONIC, &local_time[1]);
	calclock(local_time, &buf_dblwr_init_or_load_pages_time, &buf_dblwr_init_or_load_pages_count);
...
// jesnk
// origin : dberr_t buf_dblwr_init_or_load_pages(pfs_os_file_t file, const char *path) 
dberr_t buf_dblwr_init_or_load_pages_internal(pfs_os_file_t file, const char *path) {
  ...
  // jesnk
  printf("jesnk : AT buf_dblwr_init_or_load_pages_internal\n");

  /* Origin : We do the file i/o past the buffer pool */
  // jesnk : last buffer pool access..
  unaligned_read_buf = static_cast<byte *>(ut_malloc_nokey(2 * UNIV_PAGE_SIZE)); // i don't know..
  printf("jesnk : UNIV_PAGE_SIZE : %d\n", UNIV_PAGE_SIZE);
  // jesnk : maybe.. attach buf pointer to saved buf?
  read_buf = static_cast<byte *>(ut_align(unaligned_read_buf, UNIV_PAGE_SIZE));
  // ut_align : rounds up a pointer to the nearest aligned address.
  printf("read_buf : %d\n",*read_buf); // jesnk. can it  print?
  
  // dberr_t : enum of Present DB_ success or error
  /* ut_malloc_nokey :
static_cast<void*> ( \
...
//hypothesis : maybe first page of at TR_SYS_PAGE_NO is trx sys header?
// TRX_SYS_PAGE_NO : Page number of the transaction system page
  err = os_file_read(read_request, file, read_buf,
                     TRX_SYS_PAGE_NO * UNIV_PAGE_SIZE, UNIV_PAGE_SIZE);

/* func: os_file_read 
read_buf : buffer where to read
offset : file offset where to read
...
// TRX_SYS_DOUBLEWRITE : offset of the doublewrite buffer header on the trx system header page
  doublewrite = read_buf + TRX_SYS_DOUBLEWRITE; 

// jesnk : is dblw buf created?
// func : mach_read_from_4 : fetch data from 4 consecutive bytes.
// Val : TRX_SYS_DOUBLEWRITE_MAGIC :
//      4-byte magic number which shows if we already have created the doublewrite buffer
 if (mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC) ==
    ...
    // jesnk : buf__ initializing here
    buf = buf_dblwr->write_buf;
  } else { // jesnk : maybe.. this case : dblwrbuf is not completely created. so, there is no dirty data
    ut_free(unaligned_read_buf);
    return (DB_SUCCESS); //so, return DB_SUCCESS
  }


// dblwbuf space id syncronizing for fit this version
 ...
 // size
  printf("jesnk : %s, TRX_SYS_DOBULEWRITE_BLOCK_SIZE: %d, UNIV_PAGE_SIZE: %d\n", __func__, TRX_SYS_DOUBLEWRITE_BLOCK_SIZE, UNIV_PAGE_SIZE); 


  /* Read the pages from the doublewrite buffer to memory */
  // os_file_read(type, file, buf, offset, n)
 
   // block 1 : Read file , at block1 (which is starting page of Block11, size of 1 Block(64page)
  ...
  // block 2 : Read file, at block2. (which is staring page of Block2)
  err = os_file_read(
 ...
 //pthread_create()
 //pthread_join()
  //sysganda loop 128, 2 thread 64, 4 thread 32, 8 thread: 16
//  for (page_no_t i = 0; i < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2; i++) {
/*

	i_global = 0;
//  for (; i < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2; __sync_fetch_and_add(&i_gloabl, 1) {
...
*/
  ...
  for (page_no_t i = 0; i < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2; i++) {
     if (reset_space_ids) {
      ...
      // sysganda : get the page number
      // sysganda : decide the block 1 or block 2, because we hav blcok 2 (it means that two area for double write buffer)
	...
	//jesnk : might. store page to recover  later
        //jesnk : Printing_jesnk : recv_dblwr.add(page) : 0
	//printf("init_or_load : store poge to recover later\n");
...
try_again:
  ...
  // jesnk : TESTING first_free value..
  // jesnk : Printing_jesnk : first_free : 0
...
// jesnk : AT buf_dblwr_write_single_page
    ...
    bool sync)         /*!< in: true if sync IO requested */
{
  ...
  ut_a(buf_dblwr != NULL);
  // jesnk : Printing_jesnk write_single_page : 0
  // printf("buf_dblwr_write_single_page\n");
